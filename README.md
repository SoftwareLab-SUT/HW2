...
دستور fetch: این دستور تغییرات کامیت‌ها، refها و فایل‌ها را از repository ریموت دریافت می‌کند ولی هیچ تغییری روی شاخه‌های local‌ اعمال نمی‌کند. بنابراین کد ما تغییری نمی‌کند که این تفاوتش با دستور git pull است.
...
دستور clone:
clone کردن یک مخزن، درواقع کپی کردن آن مخزن است. هنگامی که یک مخزن ریموت داریم و دستور git clone‌را برای url آن مخزن می‌زنیم، یک کپی محلی از آن مخزن برای ما درست می‌شود. یک clone، تمامی فایل‌ها، شاخه‌ی master و تمامی شاخه‌های دیگر ، کامیت‌ها و ... را در بر دارد. تمامی تغییراتی که لوکال روی لپ‌تاپ خود برای این مخزن انجام می‌دهیم روی این clone انجام می‌شود و سپس این تغییرات پس از کامیت شدن روی مخزن ریموت push می‌شوند. مثلا با دستور زیر می‌توانید یک clone از همین مخزن پاسخ تمرین به صورت محلی داشته باشید:

```sh
$ git clone https://github.com/SoftwareLab-SUT/HW2.git
```
دستور rebase:
 

https://user-images.githubusercontent.com/56794518/201389148-6eaa1642-2b2c-4756-9f00-cdbe9a9777b8.mp4

این دستور به شما امکان می دهد commitها را از یک شاخه بالای شاخه‌ی هدف بازنویسی کنید. rebase در مقایسه با merge، قابلیت یک تاریخچه مخزن خطی‌تر و تمیزتر را دارد. مثلا فرضا می‌خواهید دو commit سر شاخه‌ی new-branch (که شاخه‌ای است که پیشتر از main‌ به وجود آمده) را به شاخه‌ی main ببرید. هنگامی که روی شاخه‌ی new-branch باشیم، با اجرای دستور

```sh
$ git rebase main
```
آن commitهایی که new-branch پس از به وجود آمدن از main پیدا کرد به سر تازه‌ترین commit شاخه‌ی main اضافه می‌شوند.
